...
# 4. 测试
# 提交 ISL=1024, OSL=1024 的所有结果（自动跑 CONC=4,8,32,64,128,256，~2-3小时）
./dsr1_benchmark submit "YourTeam" -isl 1024 -osl 1024

# 提交 ISL=1024, OSL=8192 的所有结果（自动跑 CONC=4,8,16,32,64,128，~2-3小时）
./dsr1_benchmark submit "YourTeam" -isl 1024 -osl 8192

# 提交 ISL=8192, OSL=1024 的所有结果（自动跑 CONC=4,8,16,32,64,128，~2-3小时）
...
| **submit -isl -osl** ⭐ | `./dsr1_benchmark submit "Team" -isl 1024 -osl 1024` | 自动测试 6 个 CONC + 提交 | ~2-3小时 | **推荐：批量测试并提交** |
| **acc** | `./dsr1_benchmark acc` | 仅准确性测试 | ~5-10分钟 | 可选：快速验证准确性 |
| **perf** | `./dsr1_benchmark perf` | 准确性 + 性能（不提交）| ~20-30分钟 | 可选：测试性能但不提交 |
| **perf -isl -osl** | `./dsr1_benchmark perf -isl 1024 -osl 1024` | 自动测试 6 个 CONC（不提交）| ~2-3小时 | 可选：批量测试但不提交 |
...
| **方式 2: 多并发** ⭐ | `./dsr1_benchmark submit "Team" -isl 1024 -osl 1024` | 6个 | ~2-3小时 | **批量测试所有CONC** |
...
# ========== 推荐：直接测试并提交（一步到位）========== 
# 4.
# 提交 ISL=1024, OSL=1024 的所有结果（自动测试 CONC=4,8,32,64,128,256，~2-3小时）
./dsr1_benchmark submit "YourTeam" -isl 1024 -osl 1024

# 提交 ISL=1024, OSL=8192 的所有结果（自动测试 CONC=4,8,16,32,64,128，~2-3小时）
./dsr1_benchmark submit "YourTeam" -isl 1024 -osl 8192

# 提交 ISL=8192, OSL=1024 的所有结果（自动测试 CONC=4,8,16,32,64,128，~2-3小时）
./dsr1_benchmark submit "YourTeam" -isl 8192 -osl 1024
```

**每条命令会自动**：
- ✅ 测试 6 个 CONC 值（4, 8, 16, 32, 64, 128或4, 8, 32, 64, 128，256）
- ✅ 运行准确性 + 性能测试
...
- **submit -isl -osl**: ~20-30分钟/CONC × 6 = **~2-3小时** ⭐ **推荐**
- **acc -isl -osl**: ~5-10分钟/CONC × 6 = **30-60分钟**（可选）
- **perf -isl -osl**: ~20-30分钟/CONC × 6 = **~2-3小时**（可选）

**全部 3 个 ISL-OSL 组合**（18 个配置）：
- **submit 模式**: ~2-3小时 × 3 = **~6-9小时** ⭐
